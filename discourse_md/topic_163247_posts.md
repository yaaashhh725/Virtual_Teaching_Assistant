...
## Error: Error: Invalid promptfooconfig.yaml: Your config must include at least 5 test assertions. [@carlton](/u/carlton) [@s.anand](/u/s.anand) [@Jivraj](/u/jivraj)
...
image description: The image shows a section from a document with instructions for creating a search API endpoint using TypeScript. It details the expected output, context about TechDocs Inc., example questions and answers, task requirements for the API, and an input field to enter the API's URL. There is also a 'Check' button.
image text: Expected output:
• The raw matched snippets printed first.
• Then a concise, streamed LLM answer citing the note verbatim.
TechDocs Inc. is building a next-generation documentation search system. The company maintains technical documentation for multiple programming languages and frameworks. Their goal is to implement a Retrieval Augmented Generation (RAG) system that can provide precise, factual answers to developer questions by referencing their documentation.
For this exercise, you'll implement a small proof-of-concept focusing on questions from the [TypeScript Book](https://github.com/basarat/typescript-book/). Your API should return relevant excerpts from the TypeScript book that contain the answer to each query.
...
A: !!
Task Requirements:
1. Create an API endpoint that accepts GET requests with a query parameter:/search?q=question\_text
2. The response should be a JSON object containing at least:
{
"answer": "string containing the relevant documentation excerpt",
"sources": "optional: references to source document"
}
3. Ensure your response text includes the exact answer required for each question
4. Enable CORS to allow requests from any origin
Enter the URL of your RAG API endpoint (e.g. http://localhost:8000/search):
