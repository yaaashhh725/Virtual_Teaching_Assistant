...
def extract_post_data(json_file_path,md_file_path):
    """
    ...
    try:
        ...
        with open(md_file_path, "r", encoding="utf-8") as mf:
            md_content = mf.read()
        post_bodies = [p for p in md_content.split('---')]
    ...
    try:
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            for i, post in enumerate(posts_data, 1):
                ...
                f.write(f"**post_id:** {post.get('post_id', '')}  \n")
                f.write(f"**author:** {post.get('username', '')}  \n")
                f.write(f"**timestamp:** {post.get('created_at', '')}\n\n")
...
def process_json_files(input_directory, input_md, output_directory=None, output_format='md'):
    """
    ...
    # Find all md files
    md_pattern = os.path.join(input_md,"*.md")
    ...
    for json_file,md_file in zip(json_files,md_files):
        ...
        posts_data,topic_slug = extract_post_data(json_file,md_file)
...
def main():
    ...
    output_dir = './mD'
    input_md_dir = './discourse_md'
    
    # Process files
    process_json_files(input_dir, input_md_dir, output_dir, output_format)
